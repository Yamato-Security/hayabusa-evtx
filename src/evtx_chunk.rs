...
use crate::evtx_record::{EvtxRecord, EvtxRecordHeader, RecordAllocation};
    ...
    pub fn iter(&mut self) -> IterChunkRecords {
        ...
        if self.settings.should_parse_empty_chunks() {
                ...
                && self.header.last_event_record_id == 0xffffffffffffffff
            {
                ...
                let found_signatures: VecDeque<usize> =
                    memmem::find_iter(self.data, &[0x2a, 0x2a, 0x00, 0x00]).collect();
